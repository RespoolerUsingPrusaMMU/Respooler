...
```sh
cmake -S . -B build \
  -DSIMAVR_ROOT=~/RespoolerUsingPrusaMMU/simavr
cmake --build build
```
...
```sh
./build/prusa_mmu_sim \
  ~/RespoolerUsingPrusaMMU/Spooler-Firmware/build/release/firmware
```
...
## v5 deterministic button tap behavior

The `tap`/`press` commands no longer use `std::chrono` host time. They are driven by the simulated ATmega32U4 cycle counter and execute three phases:

```text
Button=None   50 ms simulated time
Button=Middle requested press duration (100 ms default)
Button=None   50 ms simulated time
```

This makes repeated taps deterministic and ensures the spooler firmware has enough simulated time to debounce the release and clear its previous button-report state. Manual `hold`, `release`, and legacy `button` commands are unchanged.

## v4 control-interface reliability fix

The simulator disables both C stdio buffering and C++ iostream buffering when it
starts. This is important when VS Code redirects simulator output to
`/tmp/prusa-spooler-sim.log`.

Every FIFO command is now acknowledged before it is executed. For example:

```text
[control] tap middle
Button tap middle started: 50 ms pre-release, 100 ms press, 50 ms post-release (simulated AVR time)
Button middle pressed
Button middle released
Button tap complete
```

If the simulated AVR is stopped at a GDB breakpoint, commands remain queued in
the FIFO until the AVR is continued because the control interface is polled
between `avr_run()` calls.
